...
print("----------训练即将开始-------------")
with open('sales_data08.csv') as f1:
    df=pd.read_csv(f1)     #读入销售数据
    #data=df.iloc[:,:13].values  #取第1-13列
    original_sales=df.iloc[:,13].values  #预测数据
    #标准化系数保存
    mean=np.mean(original_sales,axis=0)
    std=np.std(original_sales,axis=0)


#读取降维数据
with open('dem_loss_sales_sig.csv') as f2:#降维后数据
    df=pd.read_csv(f2)     #读入股票数据
    #data=df.iloc[:,:13].values  #取第1-13列
    data=df.values #全部数据
...
def get_train_data(train_begin=0,train_end=19200):
    batch_index=[]
    normalized_train_data=data[train_begin:train_end]
...
def get_verify_data(verify_begin=19200,verify_end=21600):
    #data_verify
    normalized_verify_data=data[verify_begin:verify_end]
...
def get_test_data(test_begin=21600):
    #data_test
    normalized_test_data=data[test_begin:]
...
def run_verify(sess, verify_x, verify_y):
    ...
    verify_predict=np.array(predictions)
    verify_y=np.array(verify_y)
...
verify_x,verify_y=get_verify_data(verify_begin=19200,verify_end=21600)
...
def run_test(sess, test_x, test_y):
    ...
    #反标准化
    test_predict=np.array(predictions)*std+mean
...
with tf.Session() as sess:
    ...
    test_x,test_y=get_test_data(test_begin=21600)
